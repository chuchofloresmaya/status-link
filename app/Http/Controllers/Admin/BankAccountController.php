...
class BankAccountController extends Controller
{
    ...
    public function index(Notary $notary): View
    {
        ...
        return view('admin.bank-accounts.index', ['notary' => $notary, 'accounts' => $notary->bankAccounts()->with('notarialProfile')->latest()->paginate(15)]);
    }
    ...
    public function create(Notary $notary): View
    {
        ...
        return view('admin.bank-accounts.form', ['notary' => $notary, 'bankAccount' => new BankAccount, 'profiles' => $notary->notarialProfiles()->orderBy('name')->get()]);
    }

    public function store(Request $request, Notary $notary): RedirectResponse
    {
        Gate::authorize('create', BankAccount::class);
        $this->service->storeForNotary($notary, $this->validated($request, $notary));

        return to_route('admin.notaries.bank-accounts.index', $notary)->with('success', 'Cuenta bancaria creada correctamente.');
    }
    ...
    public function edit(BankAccount $bankAccount): View
    {
        ...
        return view('admin.bank-accounts.form', ['notary' => $bankAccount->notary, 'bankAccount' => $bankAccount, 'profiles' => $bankAccount->notary->notarialProfiles()->orderBy('name')->get()]);
    }

    public function update(Request $request, BankAccount $bankAccount): RedirectResponse
    {
        Gate::authorize('update', $bankAccount);
        $this->service->updateBankAccount($bankAccount, $this->validated($request, $bankAccount->notary));

        return to_route('admin.notaries.bank-accounts.index', $bankAccount->notary)->with('success', 'Cuenta bancaria actualizada.');
    }
